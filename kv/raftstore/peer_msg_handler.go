...
func (d *peerMsgHandler) HandleRaftReady() {
	...
	if len(ready.CommittedEntries) > 0 {
		...
		if err := kvWB.SetMeta(meta.ApplyStateKey(d.regionId), d.peerStorage.applyState); err != nil {
			//log.Infof("HandleRaftReady SetMeta error:%v",err.Error())
			panic(err)
		}
...
func (d *peerMsgHandler) processCommittedEntry(entry *eraftpb.Entry, wb *engine_util.WriteBatch) *engine_util.WriteBatch {
	//log.Info("processCommittedEntry...")
...
func (d *peerMsgHandler) processRequest(entry *eraftpb.Entry, msg *raft_cmdpb.RaftCmdRequest, wb *engine_util.WriteBatch) *engine_util.WriteBatch {
	...
	//according to different types of request to response
	d.replyProposal(entry, func(p *proposal) {
		...
		case raft_cmdpb.CmdType_Get:
			...
			if err != nil {
				//log.Errorf("processRequest SetMeta error:%v",err.Error())
			}
			err = wb.WriteToDB(d.peerStorage.Engines.Kv)
			if err != nil {
				//log.Errorf("processRequest WriteToDB error:%v",err.Error())
			}
			value, err := engine_util.GetCF(d.peerStorage.Engines.Kv, req.Get.Cf, req.Get.Key)
			if err != nil {
				//log.Errorf("processRequest GetCF error:%v",err.Error())
				value = nil
			}
		...
		case raft_cmdpb.CmdType_Snap:
			...
			err := wb.SetMeta(meta.ApplyStateKey(d.regionId), d.peerStorage.applyState)
			if err != nil {
				//log.Errorf("processRequest CmdType_Snap SetMeta error:%v",err.Error())
			}
			err = wb.WriteToDB(d.peerStorage.Engines.Kv)
			if err != nil {
				//log.Errorf("processRequest CmdType_Snap WriteToDB error:%v",err.Error())
			}
...
func (d *peerMsgHandler) processAdminRequest(entry *eraftpb.Entry, msg *raft_cmdpb.RaftCmdRequest, wb *engine_util.WriteBatch) {
	...
	if req.CmdType == raft_cmdpb.AdminCmdType_CompactLog {
		...
		if compactLog.CompactIndex >= applySt.TruncatedState.Index {
			...
			err := wb.SetMeta(meta.ApplyStateKey(d.regionId), applySt)
			if err != nil {
				//log.Errorf("processAdminRequest SetMeta error:%v",err.Error())
			}
			d.ScheduleCompactLog(d.RaftGroup.Raft.RaftLog.LastAppend)
		}
	}
}

func (d *peerMsgHandler) replyProposal(entry *eraftpb.Entry, handle func(*proposal)) {
...
func (d *peerMsgHandler) proposeAdminRequest(msg *raft_cmdpb.RaftCmdRequest, cb *message.Callback) {
	...
	if req.CmdType == raft_cmdpb.AdminCmdType_CompactLog {
		...
		err = d.RaftGroup.Propose(data)
		if err != nil {
			//log.Errorf("proposeAdminRequest Propose error %v", err)
		}
	}
}
